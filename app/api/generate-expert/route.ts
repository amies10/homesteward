...
export async function POST(request: NextRequest) {
  try {
    ...
    const { issueTitle, issueDescription, severity, recommendedAction, equipmentSpecs, costEstimatePro } =
      body as {
        issueTitle: string;
    ...
    const response = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `Write a contractor briefing for the following home inspection issue. This is for a homeowner to read before hiring and meeting with a professional.

Issue: ${issueTitle}
Description: ${issueDescription}
Severity: ${severity}
Recommended Action: ${recommendedAction}${equipmentSpecs?.length ? `\nTypical equipment involved: ${equipmentSpecs.join(", ")}` : ""}${costEstimatePro ? `\nEstimated professional cost: ${costEstimatePro}` : ""}
